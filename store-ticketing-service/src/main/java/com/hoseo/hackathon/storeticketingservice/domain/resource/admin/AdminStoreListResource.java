...
public class AdminStoreListResource extends EntityModel<StoreListDto> {
    public AdminStoreListResource(StoreListDto dto, Link... links) {
        ...
        if(dto.getStoreStatus() == StoreStatus.VALID)
            add(linkTo(AdminController.class).slash("storeAdmins").slash(dto.getMember_id()).slash("cancel-join").withRel("매장 승인 취소"));
        else
            add(linkTo(AdminController.class).slash("storeAdmins").slash(dto.getMember_id()).slash("permit-join").withRel("매장 승인 취소"));
